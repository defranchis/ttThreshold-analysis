...
h_res_jet2_qq_fromele=df.Histo1D(    ("h_res_jet2_qq_fromele","",50,-5,5),    "res_jet2_qq_fromele")
h_res_jet1_qq_fromele=df.Histo1D(    ("h_res_jet1_qq_fromele","",50,-5,5),    "res_jet1_qq_fromele")

h_lep_res=df.Histo1D(("h_lep_res","",50,-5,5),    "lep_res")
h_mp_res=df.Histo1D(("h_met_res","",50,-5,5),    "missing_p_res")
...
#print("mean_j1 =", mean_j1)
#print("sigma_j1 =", sigma_j1)
#print("sigma_j2 =", sigma_j2)
#print("sigma_lep =", sigma_lep)
#print("sigma_mp =",sigma_mp)
...
"missing_p","missing_p_phi","missing_p_eta",
], library="np")
...
def breit_wigner(m, MW, GW):
    """
    Returns the Breit-Wigner value for mass m
    M: nominal mass (mW)
    G: width (gW)
    """
...
def event_chi2(scales,event,mW,gW):
    ...
    sigma_sn = sigma_mp / (1.0 + mean_mp)**2  #no need 
    res = (
...
for i, event in enumerate(events):
    ...
    if not m.valid:
        ...
        #print("Fixed parameters:", m.fixed)
#        print("Step sizes (iminuit errors):")
#        for par, step in m.errors.items():
#            print(f"{par}: {step}")
        continue
    if i > 100: break
    print("converged")
