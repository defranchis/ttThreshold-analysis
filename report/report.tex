...
\newcommand{\threecm}[2][0.32]{%
  \begin{subfigure}[b]{#1\linewidth}\centering
    \includegraphics[width=\linewidth]{ecm157/#2}\caption*{\sqs=157~GeV}\end{subfigure}\hfill
  \begin{subfigure}[b]{#1\linewidth}\centering
    \includegraphics[width=\linewidth]{ecm160/#2}\caption*{\sqs=160~GeV}\end{subfigure}\hfill
  \begin{subfigure}[b]{#1\linewidth}\centering
    \includegraphics[width=\linewidth]{ecm163/#2}\caption*{\sqs=163~GeV}\end{subfigure}}

% Three-ECM panel of a kinfit_vars subdir plot.
\newcommand{\threecmkin}[2][0.32]{%
  \begin{subfigure}[b]{#1\linewidth}\centering
    \includegraphics[width=\linewidth]{kinfit_vars/ecm157/#2}\caption*{\sqs=157~GeV}\end{subfigure}\hfill
  \begin{subfigure}[b]{#1\linewidth}\centering
    \includegraphics[width=\linewidth]{kinfit_vars/ecm160/#2}\caption*{\sqs=160~GeV}\end{subfigure}\hfill
  \begin{subfigure}[b]{#1\linewidth}\centering
    \includegraphics[width=\linewidth]{kinfit_vars/ecm163/#2}\caption*{\sqs=163~GeV}\end{subfigure}}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \emph{dcber2g} (DCB with power-law left tail and exponential
        right tail) for the lepton $p$-response (post-update;
        cf.~Sec.~\ref{sec:lep-p-resp}) and for $m(\WW)-m(e^+e^-)$;
...
\subsection{Lepton $p$-response: model swap}\label{sec:lep-p-resp}
...
$p_\text{reco}/p_\text{gen}\le 1$. The pre-update model was
\codename{expleft2g} (exp-left tail $+$ power-law right), the
\emph{mirror} of the physical asymmetry. Switching to \codename{dcber2g}
(power-left $+$ exp-right) reduces the inclusive \chitndf from $\{3.7,
3.5, 3.5\}$ to $\{2.7, 1.7, 2.1\}$ at the three energies, with visibly
flatter pull distributions (Fig.~\ref{fig:lep-p-resp-compare}).

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.97\linewidth]{lep_p_resp_compare.png}
  \caption{Lepton $p$-response fit at \sqs=160~GeV: legacy
    \codename{expleft2g} (left) vs.\ updated \codename{dcber2g} (right).
    \chitndf improves from 4.5 to 1.7 and the structured pull pattern
    around the peak is removed.}
  \label{fig:lep-p-resp-compare}
\end{figure}
...
Each event is fitted with up to 8 sequential passes of Minuit2, each
seeded from the running best minimum, and the cascade stops as soon as
a pass returns Migrad status $\in\{0,1\}$:
\begin{enumerate}[label=(\arabic*),leftmargin=*]
  \item Migrad with the natural jet-prior assignment;
  \item Migrad with the jet1$\leftrightarrow$jet2 priors swapped
        (hedges against the prior fit's $p_T$ ordering disagreeing
        with the event's);
  \item Simplex pre-pass plus Migrad on natural priors;
  \item Simplex plus Migrad on swapped priors;
  \item Hesse refresh plus Migrad continuation (recovers from a
        numerically broken Davidon Hessian);
  \item Random-restart Migrad (deterministically seeded from the event
        kinematics, two random draws of jittered initial point);
  \item Combined ``Minimize'' method (Migrad$\to$Simplex$\to$Migrad);
  \item Combined ``Minimize'', swapped.
\end{enumerate}
...
The current pipeline state (commits up to \codename{f1c3a36} plus the
lep $p$-response model swap and the Migrad-step refinement) is
benchmarked on a 92k-event subset per energy point. The strict and
loose valid fractions are reported in Tab.~\ref{tab:valid}; closure
of the kinfit-derived $W$-mass against truth is reported in
Tab.~\ref{tab:closure}, and key plots in
...
\begin{table}[ht]
  ...
  \caption{Strict (\codename{kinfit\_valid}) and loose
    (\codename{kinfit\_valid\_loose}) convergence fractions.}
  \label{tab:valid}
  \begin{tabular}{lccc}\toprule
    & \sqs=157~GeV & \sqs=160~GeV & \sqs=163~GeV \\\midrule
    valid (strict) [\%] & 89.70 & 82.28 & 58.16 \\
    valid (loose)  [\%] & 95.37 & 93.19 & 86.34 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[ht]
  \centering
  \begin{subfigure}[b]{0.32\linewidth}\centering
    \includegraphics[width=\linewidth]{lnuqq/allbranches/mW_overlay_ecm157.png}
    \caption*{\sqs=157~GeV}\end{subfigure}\hfill
  \begin{subfigure}[b]{0.32\linewidth}\centering
    \includegraphics[width=\linewidth]{lnuqq/allbranches/mW_overlay_ecm160.png}
    \caption*{\sqs=160~GeV}\end{subfigure}\hfill
  \begin{subfigure}[b]{0.32\linewidth}\centering
    \includegraphics[width=\linewidth]{lnuqq/allbranches/mW_overlay_ecm163.png}
    \caption*{\sqs=163~GeV}\end{subfigure}
  \caption{$W$-mass overlay at the three centre-of-mass energies.
    Solid red is the per-event kinfit-combined $W$ mass; dashed
    blue/green are the post-fit individual leptonic and hadronic $W$
    masses; dotted lines are the pre-fit (reco) values.}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Lepton $\phi$-shift (\codename{kinfit\_pl})
        pathology.} The lepton-$\phi$ post-fit distribution is
        $\sim 19\times$ wider than the prior core, the only nuisance
        that pulls into the prior tails. The likely root cause is an
        over-tight prior that the kinematic balance does not constrain
        sufficiently.
  \item \textbf{Lepton $\theta$-shift (\codename{kinfit\_tl})
        double-peak.} A small fraction of events ($\sim 18\%$ of the
        valid sample) cluster on two preferred \codename{tl} values.
        The structure is invariant under the lepton $p$-response model
        swap, suggesting it is intrinsic to the kinematic-balance
        topology. Likely shares root cause with the
        \codename{kinfit\_pl} pathology.
...
\begin{figure}[ht]\centering\threecm{lep_p_resp.png}\caption{\codename{lep\_p\_resp} (dcber2g, post-update).}\end{figure}
...
\subsection{Lepton $p$-response: model comparison}
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.97\linewidth]{lep_p_resp_dcber2g_all.png}
  \caption{Updated \codename{dcber2g} fit at all three ECMs.
    \chitndf $\in \{2.68, 1.66, 2.10\}$ vs.\ the previous
    \codename{expleft2g} $\{3.67, 3.52, 3.52\}$.}
\end{figure}
...
\section{Post-fit nuisance pulls}\label{app:postfit}

The 16 free parameters of the kinematic fit have well-defined
posteriors that we plot per energy point with the corresponding input
prior PDF (red curve) overlaid. The green histogram is the post-fit
distribution of the named branch.

\subsection{BES nuisances}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_bes_m_minus_ecm.png}\caption{\codename{kinfit\_bes\_m\_minus\_ecm} ($\Delta_{m,\text{fit}}$).}\end{figure}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_bes_pz.png}\caption{\codename{kinfit\_bes\_pz} ($P_{z,\text{fit}}$).}\end{figure}

\subsection{Momentum scales}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_s1.png}\caption{Jet 1 momentum scale.}\end{figure}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_s2.png}\caption{Jet 2 momentum scale.}\end{figure}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_sl.png}\caption{Lepton momentum scale.}\end{figure}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_sn.png}\caption{MET (neutrino) momentum scale.}\end{figure}

\subsection{Polar-angle shifts}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_t1.png}\caption{Jet 1 $\theta$ shift.}\end{figure}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_t2.png}\caption{Jet 2 $\theta$ shift.}\end{figure}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_tl.png}\caption{Lepton $\theta$ shift (double-peak structure under investigation).}\end{figure}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_tn.png}\caption{MET $\theta$ shift.}\end{figure}

\subsection{Azimuthal-angle shifts}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_p1.png}\caption{Jet 1 $\phi$ shift.}\end{figure}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_p2.png}\caption{Jet 2 $\phi$ shift.}\end{figure}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_pl.png}\caption{Lepton $\phi$ shift (pathologically wide vs.\ prior --- see Sec.~\ref{sec:open}).}\end{figure}
\begin{figure}[ht]\centering\threecmkin{pulls/kinfit_pn.png}\caption{MET $\phi$ shift.}\end{figure}
...
\begin{figure}[ht]\centering
  \includegraphics[width=0.48\linewidth]{kinfit_vars/ecm_comparison/pulls/kinfit_bes_m_minus_ecm.png}
  \includegraphics[width=0.48\linewidth]{kinfit_vars/ecm_comparison/pulls/kinfit_bes_pz.png}
